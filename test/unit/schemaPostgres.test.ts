...
describe('PostgresDatabase', () => {
    ...
    describe('query', () => {
        it('calls postgres query', () => {
            ...
            assert.strictEqual(db.query.getCall(0).args[0], 'SELECT * FROM TEST')
        })
    })
    describe('getEnumTypes', () => {
        it('writes correct query with schema name', () => {
            PostgresProxy.getEnumTypes('schemaName')
            assert.strictEqual(db.each.getCall(0).args[0],
                'select n.nspname as schema, t.typname as name, e.enumlabel as value ' +
                'from pg_type t join pg_enum e on t.oid = e.enumtypid ' +
                'join pg_catalog.pg_namespace n ON n.oid = t.typnamespace ' +
                'where n.nspname = \'schemaName\' ' +
                'order by t.typname asc, e.enumlabel asc;')
            assert.deepStrictEqual(db.each.getCall(0).args[1], [])
        })
        ...
        it('handles response from db', async () => {
            ...
            assert.deepStrictEqual(enums, { name: ['value1', 'value2'] })
        })
    })
    describe('getTableDefinition', () => {
        it('writes correct query', () => {
            ...
            assert.deepStrictEqual(db.each.getCall(0).args[1], ['tableName', 'schemaName'])
        })
        it('handles response from db', async () => {
    ...
    describe('getTableTypes', () => {
        ...
        it('gets custom types from enums', async () => {
            ...
            assert.deepStrictEqual(PostgresDBReflection.mapTableDefinitionToType.getCall(0).args[1], ['enum1', 'enum2'])
        })
        it('gets table definitions', async () => {
            ...
            await PostgresProxy.getTableTypes('tableName', 'tableSchema')
            assert.deepStrictEqual(PostgresProxy.getTableDefinition.getCall(0).args, ['tableName', 'tableSchema'])
            assert.deepStrictEqual(PostgresDBReflection.mapTableDefinitionToType.getCall(0).args[0], { table: {
                udtName: 'name',
                nullable: false
            }})
        })
    ...
    describe('getSchemaTables', () => {
        it('writes correct query', () => {
            ...
            assert.strictEqual(db.map.getCall(0).args[0],
                'SELECT table_name ' +
                'FROM information_schema.columns ' +
                'WHERE table_schema = $1 ' +
                'GROUP BY table_name')
            assert.deepStrictEqual(db.map.getCall(0).args[1], ['schemaName'])
        })
        it('handles response from db', async () => {
    ...
    describe('mapTableDefinitionToType', () => {
        describe('maps to string', () => {
            it('bpchar', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td,[],options).column.tsType, 'string')
            })
            it('char', () => {
                const td: TableDefinition = {
                    column: {
        ...
        describe('maps to number', () => {
            it('int2', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td,[],options).column.tsType, 'number')
            })
            it('int4', () => {
                const td: TableDefinition = {
                    column: {
        ...
        describe('maps to boolean', () => {
            it('bool', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td,[],options).column.tsType, 'boolean')
            })
        })
        describe('maps to Object', () => {
            it('json', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td,[],options).column.tsType, 'Object')
            })
            it('jsonb', () => {
                const td: TableDefinition = {
                    column: {
        ...
        describe('maps to Date', () => {
            it('date', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td,[],options).column.tsType, 'Date')
            })
            it('timestamp', () => {
                const td: TableDefinition = {
                    column: {
        ...
        describe('maps to Array<number>', () => {
            it('_int2', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td,[],options).column.tsType, 'Array<number>')
            })
            it('_int4', () => {
                const td: TableDefinition = {
                    column: {
        ...
        describe('maps to Array<boolean>', () => {
            it('_bool', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td, ['CustomType'], options).column.tsType, 'Array<boolean>')
            })
        })
        describe('maps to Array<string>', () => {
            it('_varchar', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td, ['CustomType'], options).column.tsType, 'Array<string>')
            })
            it('_text', () => {
                const td: TableDefinition = {
                    column: {
        ...
        describe('maps to Array<Object>', () => {
            it('_json', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td,[],options).column.tsType, 'Array<Object>')
            })
            it('_jsonb', () => {
                const td: TableDefinition = {
                    column: {
        ...
        describe('maps to Array<Date>', () => {
            it('_timestamptz', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td,[],options).column.tsType, 'Array<Date>')
            })
        })
        ...
        describe('maps to custom', () => {
            it('CustomType', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td, ['CustomType'], options).column.tsType, 'CustomType')
            })
        })
        describe('maps to any', () => {
            it('UnknownType', () => {
                ...
                assert.strictEqual(PostgresDBReflection.mapTableDefinitionToType(td, ['CustomType'], options).column.tsType, 'any')
            })
        })
    })
})
